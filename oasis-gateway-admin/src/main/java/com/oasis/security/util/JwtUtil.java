...
public class JwtUtil {
    ...
    public JwtUtil(OasisSecurityProperties oasisSecurityProperties) {
        this.oasisSecurityProperties = oasisSecurityProperties;
        key = new SecretKeySpec(Base64.getDecoder().decode(oasisSecurityProperties.getJwt().getKey()), "HmacSHA512");
        refreshKey = new SecretKeySpec(Base64.getDecoder().decode(oasisSecurityProperties.getJwt().getRefreshKey()), "HmacSHA512");
    }
    ...
    public String createAccessToken(UserDetails userDetails) {
        return createJWTToken(userDetails, oasisSecurityProperties.getJwt().getAccessTokenExpireTime());
    }

    public String createRefreshToken(UserDetails userDetails) {
        return createJWTToken(userDetails, oasisSecurityProperties.getJwt().getRefreshTokenExpireTime(), refreshKey);
    }
